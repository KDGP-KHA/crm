    ...
    */
    ...
    $(document, "table", "form").on("click",
        ...
        function () {
            ...
            var asideDismiss = $(this).attr("data-aside-dismiss") == "true";
            var belowNav = $(this).attr("data-aside-belowNav") == "true";
            var asideWidth = $(this).attr("data-aside-width");

            var notLayout = $(this).attr("data-not-layout");
            if (typeof notLayout == "undefined") {
                            ...
                            if (_isJson(data)) {
                                ...
                                if (typeof asidePlacement != "undefined") {
                                    ...
                                    if (asideWidth == undefined) {
                                        ...
                                        $("#" + idModal + " .modal-dialog").css("width", asideWidth);
                                    }
                                    //.addClass('w-auto');
                                }
                                $("#" + idModal).modal("show");
                ...
                $("#ModalContent").append("<div></div>")
                $("#ModalContent div").load(this.href, function (data, textStatus, xhr) {
                    if (xhr.status === 401) {
                        window.location.href = window.location.origin + "/Account/Login";
                    } else if (xhr.status === 404) {
                        window.location.href = window.location.origin + "/Error/NotFound";
...
function showNotify(title, icon, message, url, target, type, placement) {
    ...
    //$.aceToaster.add({
    //    placement: 'tr',
    //    body: '<div class="position-tl h-102 border-l-4 brc-' + clssName + ' mt-n1px"></div><i class="align-self-center ml-auto fa ' + icon + ' mr-3 text-180 text-' + clssName + '"></i><span class="align-self-center text-' + clssName + '-d2 text-120">' + message + '</span><button type="button" class="close align-self-center ml-auto text-danger text-150" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">×</span></button>',
    //    delay: 4000,
    //    close: false,
    //    bodyClass: 'd-flex bgc-' + clssName + '-l4 brc-' + clssName + '-m4 border-1 border-l-0 pl-3 radius-l-0',
    //    headerClass: 'd-none',
    ...
    //    progressReverse: true,
    //    //sticky: true,
    //    title: '<h3 class="text-success-d1 text-130">{0}</h3>'.format(title),
    //    body: message,
    //    closeClass: 'btn btn-h-' + clssName + ' btn-xs px-2 py-0 text-150 position-tr mr-n25 mt-n3',
    //    delay: 50000,
    //    className: 'bgc-' + clssName + '-tp2 radius-2px border-none border-l-4 brc-' + clssName + ' px-3 shadow',
    //    headerClass: 'bg-transparent border-0 text-' + clssName + '-m3 text-115 mt-35',
...
function _initColTable(settings) {
    ...
    if (dataTable.hasClass('none-init-cols')) return;
...
function _initValidForm(formId, arrIgnore) {
    ...
    var $errorClass = 'form-error text-danger-m2';
    ...
    var $formValid = $(formId).validate({
        ...
        errorPlacement: function (error, element) {
            ...
            error.prepend('<i class="fas fa-exclamation-circle text-danger-m1 text-100 mr-1 ml-2"></i>');
            ...
            //else if (element.is('.chosen')) {
                ...
                if (element.parents('.d-inline-flex').length > 0 || element.parents('.d-flex')) {
                    error.addClass('d-inline-block').insertAfter(element.parent());
                }
                else {
